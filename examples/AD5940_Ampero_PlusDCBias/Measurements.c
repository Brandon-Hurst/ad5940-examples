...
static AD5940Err AppAMPSeqMeasureGen(void)
{
  ...
  // Error-checking
  error = AD5940_SEQGenFetchSeq(&pSeqCmd, &SeqLen);
  if (error != AD5940ERR_OK) {return error;} //stop measurements if we encountered an error; else continue
	
	/************* VOLTAGE MEASUREMENT CHUNK ****************************/
	//Configure LPTIA switches for voltage measurement 
	AD5940_WriteReg(REG_AFE_LPTIASW0, LPTIASW(2)|LPTIASW(10)|LPTIASW(12)|LPTIASW(13)|LPTIASW(15));
	
	// Configure ADCMUX to take in the DC Bias voltage inputs
	adcbase_cfg.ADCMuxP = ADCMUXP_VBIAS0; // Write the ADC MuxP select
	adcbase_cfg.ADCMuxN = ADCMUXN_VZERO0; // Write the ADC MuxN select
  adcbase_cfg.ADCPga = AppAMPCfg.ADCPgaGain; // Write the desired ADC PGA gain
	AD5940_ADCBaseCfgS(&adcbase_cfg);
  ...
  AD5940_SEQGpioCtrlS(0);
	...
	//Reset the ADCMux and the LPTIA switches
	AD5940_WriteReg(REG_AFE_LPTIASW0, LPTIASW(5)|LPTIASW(2)|LPTIASW(4)|LPTIASW(12)|LPTIASW(13)); 
	adcbase_cfg.ADCMuxN = ADCMUXN_VZERO0; // Write the ADC MuxN select
	adcbase_cfg.ADCMuxP = ADCMUXP_AIN4; // Write the ADC MuxP select;
  adcbase_cfg.ADCPga = AppAMPCfg.ADCPgaGain; // Write the desired ADC PGA gain
	AD5940_ADCBaseCfgS(&adcbase_cfg);
	/************* END VOLTAGE MEASUREMENT CHUNK ****************************/
...
static AD5940Err AppAMPDataProcess(int32_t * const pData, uint32_t *pDataCount)
{
  ...
  {
    pData[i] &= 0xffff;
    if (i % 2) //expect odd samples to be voltage
		{
			pOut[i] = AppAMPCalcVoltage(pData[i]);
		}
		else
		{
...
float AppAMPCalcVoltage(uint32_t ADCcode)
{
  ...
  {
    case ADCPGA_1:
      fVolt = ((float)(tmp)/32768)*(AppAMPCfg.ADCRefVolt/1.0f)*kFactor;
      break;
    case ADCPGA_1P5:
      fVolt = ((float)(tmp)/32768)*(AppAMPCfg.ADCRefVolt/1.5f)*kFactor;
      break;
    case ADCPGA_2:
      fVolt = ((float)(tmp)/32768)*(AppAMPCfg.ADCRefVolt/2.0f)*kFactor;
      break;
    case ADCPGA_4:
      fVolt = ((float)(tmp)/32768)*(AppAMPCfg.ADCRefVolt/4.0f)*kFactor;
      break;
    case ADCPGA_9:
      fVolt = ((float)(tmp)/32768)*(AppAMPCfg.ADCRefVolt/9.0f)*kFactor;
      break;
  } 
  return fVolt;
}
